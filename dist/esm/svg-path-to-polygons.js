    ...
    makeAbsolute(parseSVG(svgPathString)).forEach((cmd) => {
        switch (cmd.code) {
            ...
            case "Z":
                ...
                break;
            case "C":
                sampleCubicBezier(cmd.x0, cmd.y0, cmd.x1, cmd.y1, cmd.x2, cmd.y2, cmd.x, cmd.y, tolerance2, addPoint);
                addPoint(cmd.x, cmd.y);
                break;
            case "S": {
                let x1 = 0, y1 = 0;
                ...
                sampleCubicBezier(cmd.x0, cmd.y0, x1, y1, cmd.x2, cmd.y2, cmd.x, cmd.y, tolerance2, addPoint);
                addPoint(cmd.x, cmd.y);
                break;
            }
            case "Q": {
                const [cp0, cp1, cp2, cp3] = quadraticToCubicBezier(cmd);
                sampleCubicBezier(cp0[0], cp0[1], cp1[0], cp1[1], cp2[0], cp2[1], cp3[0], cp3[1], tolerance2, addPoint);
                addPoint(cp3[0], cp3[1]);
                break;
            }
            case "A": {
                const curves = ellipticArcToCubicBezierCurves([cmd.x0, cmd.y0], [cmd.x, cmd.y], [cmd.rx, cmd.ry], cmd.xAxisRotation, cmd.largeArc, cmd.sweep);
                curves.forEach(([p0, p1, p2, p3], i) => {
                    if (i === 0)
                        addPoint(p0[0], p0[1]);
                    sampleCubicBezier(p0[0], p0[1], p1[0], p1[1], p2[0], p2[1], p3[0], p3[1], tolerance2, addPoint);
                    addPoint(p3[0], p3[1]);
                });
                break;
            }
